    ...
    #  tags$script(type = "IN/Share",`data-url` = url2)),
      ...
      #Add a season filter. Users can filter the last 3 NBAseasons (17-18, 18-19, 19-20)
       fluidRow(width = "100%", align = "center",
                  ...
                  pickerInput(
                    ...
                    selected = sort(unique(sche$Season)) %>% tail(1),
      ...
      userBox(
        ...
        title = userDescription(
          ...
          title = "airball",
          #subtitle = h6(HTML("Check out my R package")),
          type = 2,
          image = "https://raw.githubusercontent.com/josedv82/airball/master/man/images/airballlogo.PNG",
        ...
        h5(HTML("After creating this app, I published {airball}. <br> 
                An R package to quickly extract NBA <br> schedule metrics and more since the 1947 <br> season.")),
        
        footer = tags$a(href = "https://github.com/josedv82/airball", "Go to airball", style = "color:dodgerblue; text-align: left")
    ...
    tabName = "reading",
      ...
      dropdown(
        ...
        #creates an input for users to filter articles by type. i.e scientific or media articles
          ...
          selected = c("Research", "Media / Others"),
          multiple = TRUE), 
...
#right side bar. This code sets user interface for the right side bar#####
  ...
  #right side bar provides several inputs for users to customise different metrics related to schedule loads and density
    controlbar = dashboardControlbar(
      ...
      skin = "dark", #sets the background color of the bar
      ...
      controlbarItem(
        ...
        sliderTextInput("away","Away Games", choices = seq(from = 1, to = 5, by = 0.5), selected = 2.5, grid = T),
        tags$hr(),
        
        #input for travel load factors
        tags$h4("Travel", style = "color:#e48b38"),
        sliderTextInput("travelyes","Game with travel", choices = seq(from = 1, to = 5, by = 0.5), selected = 2.5, grid = T),
        ...
        sliderTextInput("east","Direction Eastbound", choices = seq(from = 1, to = 5, by = 0.5), selected = 2, grid = T),
        sliderTextInput("west","Direction Westbound", choices = seq(from = 1, to = 5, by = 0.5), selected = 3, grid = T),
      ...
      controlbarItem(
        ...
        sliderTextInput("1east","1 Zone East", choices = seq(from = 1, to = 5, by = 0.5), selected = 2, grid = T),
        sliderTextInput("2east","2 Zones East", choices = seq(from = 1, to = 5, by = 0.5), selected = 3, grid = T),
        sliderTextInput("3east","3 Zones East", choices = seq(from = 1, to = 5, by = 0.5), selected = 4, grid = T),
        sliderTextInput("1west","1 Zone West", choices = seq(from = 1, to = 5, by = 0.5), selected = 2.5, grid = T),
        sliderTextInput("2west","2 Zone West", choices = seq(from = 1, to = 5, by = 0.5), selected = 3.5, grid = T),
        sliderTextInput("3west","3 Zone West", choices = seq(from = 1, to = 5, by = 0.5), selected = 4.5, grid = T)
      ...
      controlbarItem(
        ...
        sliderTextInput("2day","2 Days", choices = seq(from = 1, to = 5, by = 0.5), selected = 3.5, grid = T),
        sliderTextInput("3day","3 Days", choices = seq(from = 1, to = 5, by = 0.5), selected = 2, grid = T),
        sliderTextInput("4day","4 Days", choices = seq(from = 1, to = 5, by = 0.5), selected = 1.5, grid = T),
...
#Footer. The footer bar displayed a the bottom of the dashboard###########
  ...
  footer = dashboardFooter(
    ...
    right = HTML(paste(img(src = "hexlogo.png", width = "35px", height = "33px"), 
    ...
    left = HTML(paste(icon = icon("copyright"), tags$span("2022. Created by Jose Fernandez", style = "font-family: Arial; color: grey; font-size: 16px"),
  ...
  observeEvent(input$welcome, {
    ...
    showModal(
      ...
      modalDialog(
        ...
        tags$img(src = "https://cdn-icons.flaticon.com/png/512/4482/premium/4482449.png?token=exp=1646901392~hmac=92c7dddf3f48fab19868e858450e2d05", width = "60px", height = "60px", align = "left"),
        ...
        tags$img(src = "https://cdn-icons.flaticon.com/png/512/2511/premium/2511651.png?token=exp=1646901566~hmac=75b5178a2b673d65674945ee8f350137", width = "60px", height = "60px", align = "left"),
        ...
        tags$img(src = "https://cdn-icons.flaticon.com/png/512/4258/premium/4258436.png?token=exp=1646901634~hmac=21d24ea7fec9d90259994b96b008e5d4", width = "60px", height = "60px", align = "left"),
        ...
        tags$img(src = "https://cdn-icons.flaticon.com/png/512/3822/premium/3822520.png?token=exp=1646901684~hmac=6353cd7d5776d3ab9d9d8c65abbe42e3", width = "60px", height = "60px", align = "left"),
        ...
        tags$img(src = "https://cdn-icons.flaticon.com/png/512/3598/premium/3598270.png?token=exp=1646901750~hmac=9e957bcd824f76412d416d72b70f4ad6", width = "60px", height = "60px", align = "left"),
...
#much weight to give to each type of game and the rolling window for the average
  ...
  sche1 <- reactive({
     ...
     sche %>% 
      ...
      #correction for bubble game
      mutate(City = ifelse(Arena == "Disney Resort", "Orlando", City)) %>%
      mutate(Location = ifelse(Arena == "Disney Resort", "Home", Location)) %>%
      ...
      mutate(Correct = ifelse(Season == "2019-20" & Month %in% c("July","August"), "a", "b")) %>% #correct moving avr for bubble
      
      group_by(Correct, Team) %>%
      
      mutate(MovIndex = round(roll_meanr(Normalized, n = input$rolling, align = "right", fill = 0, na.rm = T),1)) %>%
      select(-Correct) %>% ungroup()
  ...
  cities <- reactive({
    ...
    acities %>%
      ...
      #correct for Toronto playing in Tampa in 2021
      mutate(City = ifelse(Season == "2020-21" & Team == "Toronto Raptors" & Location == "Home", "Tampa", City)) %>%
      mutate(Latitude = ifelse(Season == "2020-21" & Team == "Toronto Raptors" & Location == "Home", 27.96, Latitude)) %>%
      mutate(Longitude = ifelse(Season == "2020-21" & Team == "Toronto Raptors" & Location == "Home", -82.48, Longitude)) %>%
      dplyr::distinct() %>%
      ...
      mutate(
        destLat = ifelse(is.na(destLat), Latitude, destLat),
        destLon = ifelse(is.na(destLon), Longitude, destLon)
      ) %>%
      mutate(Route = paste(ifelse(is.na(lag(City)), City, lag(City)), City, sep = " - ")) %>%
      ...
      mutate(Arena = ifelse(City == "Tampa", "Amalie Arena", Arena)) %>% #home arena for raptors
...
###################################################################################################
  ...
  output$daily_games <- DT::renderDataTable({
    ...
    validate(need(a$Game != "", "No other games are played on this date"))
  ...
  output$map_plot <- renderPlot({
    ...
    maps::map("world", regions=c("usa"), fill=T, col="#17202a", bg="#343e48", ylim=c(21.0,50.0), xlim=c(-130.0,-65.0))
  ...
  #Dynamically display the name of the city where the game is helps
  ...
  #Dynamically display the name of the arena where the game is played
 ...
 output$shotchart <- renderPlot({
   ...
   courtImg <- download.file("https://miro.medium.com/max/1838/1*2QjYg-iJ54sT1ywAoVxtaw.png", destfile = 'mypng.png', mode = 'wb')
   court <- readPNG("mypng.png")
   playerImg <- download.file(unique(aa22()$Image), destfile = 'mypla.png', mode = 'wb')
   plaIMG <- readPNG("mypla.png")
   ...
   ggplot(aa22() %>% filter(Zone != "Restricted Area"), aes(x=locX, y=locY)) + 
     ...
     annotation_raster(court, -250, 250, -50, 420) +
     ...
     annotation_raster(plaIMG, xmin = 120, xmax = 240, ymin = 290, ymax = 410) +
     
     annotate("text", x = -150, y = 420, label = "@NBAGameDensity", color = "grey10", fill = "steelblue4", size = 5) +
...
###################################################################################################
  ...
  output$reads <- DT::renderDataTable({
    ...
    as.datatable(art, 
                 ...
                 escape = FALSE,
                 caption = HTML("The following references are specific to Basketball / NBA. Topics include jet-lag, travel, sleep, home-court advantage, schedule density, etc. | Last update: March 2022 |"),
                 options = list(dom = 'Bt',
                                bSort=T,
                                buttons = c('copy', 'csv', 'excel', 'pdf', 'print', I('colvis')),
                                pageLength = 500,
                                columnDefs = list(list(width = '150px', targets = c(4,5))),
